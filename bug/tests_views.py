...
class TestBugViews(TestCase):
    ...
    def test_bug_edit(self):
        ...
        self.assertEqual(new_updated_bug.title,
                         'test_bug_changed')
        self.assertEqual(new_updated_bug.description,
                         'test_description')
        self.assertEqual(new_updated_bug.status,
                         'Doing')
    ...
    def test_add_a_bug_view(self):
        response = self.c.get('/bug/add_bug/{0}'
                              .format(self.bug.id), follow=True)
        self.assertEqual(response.status_code, 200)
